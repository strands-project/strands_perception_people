...
## Running on robot
These launch files will make use of the fixed ground plane which is just rotated according to the PTU tilt and the robot odometry instead of the visual odometry. Additionally, the datacentre is assumed to run and therefore the `with_datacentre` parameter is set to `true` and the nodes will querry the config parameters from the datacentre. The standalone version on the other hand uses the provided config files. If you don not want to run the datacentre on the robot but launch the ppl perception, run with `with_datacentre:=false`.
